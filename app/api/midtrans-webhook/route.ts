...
export async function POST(req: Request) {
  try {
    ...
    if (data.transaction_status === 'settlement' || data.transaction_status === 'capture') {
      ...
      // ==========================================
      // SKENARIO 2: DAFTAR BARU
      // ==========================================
      else if (orderId.startsWith('ASK-')) {
        // AMBIL ID UUID UTUH DARI KANTONG MIDTRANS
        const fullRestoId = data.custom_field1; 

        if (!fullRestoId) {
           throw new Error('ID Resto tidak ditemukan! Pastikan file transaction/route.ts sudah mengirim custom_field1');
        }

        const expiredDate = new Date();
        expiredDate.setDate(expiredDate.getDate() + 30);

        // GUNAKAN .eq KARENA ID-NYA SUDAH SAMA-SAMA UUID (ANTI MELEDAK)
