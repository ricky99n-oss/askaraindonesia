...
function ResetPasswordForm() {
      ...
      const timer = setTimeout(() => {
        setIsVerifying((prev) => {
          if (prev) {
            ...
            setMessage({ type: 'error', text: 'Akses ditolak. Link ini tidak membawa kode verifikasi yang valid.' });
            return false;
          }
          return prev;
        });
  ...
  const verifyToken = async (token: string) => {
    ...
    if (error) {
      setIsTokenValid(false);
      setMessage({ type: 'error', text: 'Link ini sudah tidak valid atau sudah kadaluarsa. Silakan request Lupa Password lagi dari Aplikasi Kasir.' });
    } else {
      setIsTokenValid(true);
      setMessage({ type: 'info', text: 'Link valid! Silakan buat password baru Anda.' });
    }
    setIsVerifying(false);
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: error.message || 'Gagal menyimpan password. Sesi mungkin telah habis.' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white max-w-md w-full rounded-2xl shadow-xl overflow-hidden">
      {/* Header - Sudah pakai bg-linear-to-r sesuai rekomendasi Tailwind baru */}
      <div className="bg-linear-to-r from-[#4A00E0] to-[#8E2DE2] p-8 text-center text-white">
        ...
        <p className="text-purple-100 text-sm">Pemulihan akses akun Askara Smart POS Anda.</p>
      </div>

      <div className="p-8">
        {isVerifying ? (
          <div className="text-center py-6">
            <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#4A00E0] mx-auto mb-4"></div>
            <p className="text-gray-600 font-medium">Memeriksa kevalidan link...</p>
          </div>
        ) : (
          <>
            {message.text && (
...
// Wrapper Utama yang benar
export default function ResetPasswordPage() {
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
      <Suspense fallback={<div className="animate-pulse font-bold text-[#4A00E0]">Memuat form...</div>}>
        <ResetPasswordForm />
      </Suspense>
    </div>
  );
